...
const SliderManager = () => {
  ...
  const fetchSliders = async () => {
    try {
      const response = await axios.get(
        import.meta.env.VITE_API_BASE + "sliders",
        token
      );
  ...
  const handleCreateSlider = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/api/v1/sliders",
        formData,
        {
  ...
  return (
    <div className="container">
      ...
      <form
        onSubmit={editingSliderId ? handleUpdateSlider : handleCreateSlider}
      >
        <div className="mb-3">
          <label>Name:</label>
          <input
        ...
        {sliders.map((slider) => (
          <div className="col-md-4" key={slider.id}>
            <div className="card mb-3">
              <img
                src={`http://localhost:8000/${slider.image}`}
